...
#    auto-mergeado — as linhas tocam regiões disjuntas; 3 conflitos
#    resolvidos por COMPOSIÇÃO, nunca por escolha:
#    ICLOUD_BASE segue env-overridable (fleet/2.3, load-bearing para
#    fixtures de teste) E OUT_DIR usa HOST_ID serial-suffixed (2.4,
#    anti-colisão de $HOST clonado via Migration Assistant/TM).
#  · cmd_fleet lê `.host_id // .host` do latest.json — tolera schema
#    antigo (v2.2/v2.3 ainda em produção no iCloud) e herda as pastas
#    HOST_ID automaticamente pelo glob; _fleet/ segue fora do glob.
#  · Versões 2.5–2.6 puladas (referenciadas em planejamento, nunca
#    materializadas — a numeração salta de 2.4 para 2.7).
...
# v2.7: união — ICLOUD_BASE env-overridable (fleet/2.3, load-bearing p/ fixtures)
...
cmd_fleet() {
  ...
  for f in "$base"/*/latest.json; do
    ...
    host="$(jq -r '.host_id // .host // ""' "$f" 2>/dev/null)" || continue  # v2.7: host_id desambigua LocalHostName clonado (2.4); fallback .host p/ latest.json v2.3
